<script setup>
import { reactive, ref, onMounted, onBeforeMount, computed  } from "vue";
import { userStore } from "../store/user";
import { useRoute, useRouter } from 'vue-router';
import navBar from '../components/nav.vue'

const userData = userStore();

const router = useRouter(); 

const showPassword = ref(false);
const showConfirmPassword = ref(false);
const contactNumber = ref('');
const resgisterAction = ref(true);
const accountCreated = ref(false);
const verification = ref(false);
const user_id = ref();
const EmailVerification = ref(true);
const changePassword = ref(false);
const showPhone = ref(true);
const useUsernameRecovery = ref(true);

const countdownMinutes = 3;
const countdownTime = ref(countdownMinutes * 60);
let countdownInterval;

const oneTimePassword = ref(['', '', '', '', '', '']);


const submitEmail = async (event) => {
  event.preventDefault();
  userData.submitRevoceryEmail().then(e => {
   if(e.data.status =='success'){
     EmailVerification.value = false;
     contactNumber.value = e.data.contact;
     user_id.value = e.data.user_id;
   }
  });
};

const sendOtp = () => {
  verification.value = true
  userData.sendRecoveryOtp(user_id.value)
  countdownInterval = setInterval(() => {
    countdownTime.value -= 1;
    if (countdownTime.value <= 0) {
      clearInterval(countdownInterval);

      // Perform any action you want after the countdown ends
    }
  }, 1000);
  setTimeout(() => {
    verification.value = false;
    clearInterval(countdownInterval);
    countdownTime.value = countdownMinutes * 60;
  }, 180000);
}



const sendVerification = () => {
  const userOtp = oneTimePassword.value.join('');
  userData.submitUserOtp(userOtp).then(e => {
    if(e == 'verified'){
      verification.value = false
      EmailVerification.value = false;
      changePassword.value = true;
      if(userData.registerMessage == 'success'){
        toggleAction();
        accountCreated.value = true 
      }
      clearInterval(countdownInterval);
      countdownTime.value = countdownMinutes * 60;
    }
  });;
}

const clearErrorMessage = () => {
    EmailVerification.value = true;
    changePassword.value = false;
    userData.user = {};
    userData.user_password = {};
    contactNumber.value = ''
    userData.email.status = ''
    userData.user_password.status = ''
}


const submitChangePassword = () => {
    if(userData.user_password.password == userData.user_password.confirm_password){
        userData.user_password.status = ''
        userData.recoveryChangePassword(user_id.value).then(response => {
            if(response.data.status == 'success'){
                clearErrorMessage();
                router.push('/login');
            }
        });
    }else{
        userData.user_password.status = 'failed';
        userData.user_password.message = 'Password not match!'
    }
}

const passwordType = computed(() => showPassword.value ? 'text' : 'password');
const confirmpasswordType = computed(() => showConfirmPassword.value ? 'text' : 'password');

const isNumber = function(evt) {
  evt = (evt) ? evt : window.event;
  var charCode = (evt.which) ? evt.which : evt.keyCode;
  if ((charCode > 31 && (charCode < 48 || charCode > 57)) && charCode !== 46) {
    evt.preventDefault();
    return false;
  } else {
    return true;
  }
}


const tryAnotherWay = () => {
  useUsernameRecovery.value = !useUsernameRecovery.value;
  showPhone.value = !showPhone.value;
  userData.user = {}
  if(useUsernameRecovery.value == true){
    userData.user.method = 'username'
  }else{
    userData.user.method = 'phonenumber'
  }
  userData.email.status = '';
}

onMounted(() => {
    clearErrorMessage();
    if(useUsernameRecovery.value == true){
      userData.user.method = 'username'
    }
});

</script>

<template>
    <navBar/>
    <div class="row vh-100">
      <div class="col-md-6 pl-0" >
        <img src="../assets//images/about.jpg" alt="Salon" class="h-100" />
      </div>
      <div class="col-md-6 login-form d-flex align-items-center justify-content-center">
        <div class="row align-items-center justify-content-center flex-column" v-if="EmailVerification">
            <div class="alert alert-danger" role="alert" v-if="userData.email.status == 'failed'">
                {{ userData.email.message }}
              </div>
            <div class="form-input">
              <div class="alert alert-info" role="info" v-if="accountCreated">
                Account has been successfully created.
              </div>
              <h2>Recover Account</h2>
    
                <form @submit.prevent="submitEmail">
              
                <div class="form-group" v-if="useUsernameRecovery">
                    <label for="username">Username</label>
                    <input type="email" class="form-control" v-model="userData.user.user_email" placeholder="Enter username">
                </div>

                <div class="form-group" v-else>
                    <label for="username">Phone</label>
                    <input type="text" class="form-control" v-model="userData.user.phone" placeholder="Enter number" @keypress="isNumber($event)" maxlength="11" requried>
                </div>

                <span class="try-another" @click="tryAnotherWay()">  {{ showPhone ? 'Use Phone Number Instead' : 'Use Email Instead' }}</span>

                <button type="submit" class="btn login-btn mb-2 mt-2" @click="login">Submit</button>
              
              </form>
             
        </div>
        </div>
        <div class="row align-items-center justify-content-center flex-column" v-else-if="changePassword">
            <div class="alert alert-danger" role="alert" v-if="userData.user_password.status == 'failed'">
                {{ userData.user_password.message }}
              </div>

            <div class="form-input">
           
              <div class="alert alert-info" role="info" v-if="accountCreated">
                Account has been successfully created.
              </div>
              <h2>Change Password</h2>
    
                <!-- <form @submit.prevent="submitChangePassword"> -->
                <form>
                  
                <div class="form-group password">
                    <label for="password">Password</label>
                    <input :type="passwordType" class="form-control" v-model="userData.user_password.password" placeholder="Enter password" minlength="8" maxlength="16" required>
                    <label class="showpassword-container">
                    <input type="checkbox" checked="checked" v-model="showPassword" required>
                      <svg class="eye" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512"><path d="M288 32c-80.8 0-145.5 36.8-192.6 80.6C48.6 156 17.3 208 2.5 243.7c-3.3 7.9-3.3 16.7 0 24.6C17.3 304 48.6 356 95.4 399.4C142.5 443.2 207.2 480 288 480s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C433.5 68.8 368.8 32 288 32zM144 256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64 64c-7.1 0-13.9-1.2-20.3-3.3c-5.5-1.8-11.9 1.6-11.7 7.4c.3 6.9 1.3 13.8 3.2 20.7c13.7 51.2 66.4 81.6 117.6 67.9s81.6-66.4 67.9-117.6c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3z"></path></svg>
                      <svg class="eye-slash" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 640 512"><path d="M38.8 5.1C28.4-3.1 13.3-1.2 5.1 9.2S-1.2 34.7 9.2 42.9l592 464c10.4 8.2 25.5 6.3 33.7-4.1s6.3-25.5-4.1-33.7L525.6 386.7c39.6-40.6 66.4-86.1 79.9-118.4c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C465.5 68.8 400.8 32 320 32c-68.2 0-125 26.3-169.3 60.8L38.8 5.1zM223.1 149.5C248.6 126.2 282.7 112 320 112c79.5 0 144 64.5 144 144c0 24.9-6.3 48.3-17.4 68.7L408 294.5c8.4-19.3 10.6-41.4 4.8-63.3c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3c0 10.2-2.4 19.8-6.6 28.3l-90.3-70.8zM373 389.9c-16.4 6.5-34.3 10.1-53 10.1c-79.5 0-144-64.5-144-144c0-6.9 .5-13.6 1.4-20.2L83.1 161.5C60.3 191.2 44 220.8 34.5 243.7c-3.3 7.9-3.3 16.7 0 24.6c14.9 35.7 46.2 87.7 93 131.1C174.5 443.2 239.2 480 320 480c47.8 0 89.9-12.9 126.2-32.5L373 389.9z"></path></svg>
                    </label>
                </div>
                <div class="form-group password">
                    <label for="password">Confirm Password</label>
                    <input :type="confirmpasswordType" class="form-control" v-model="userData.user_password.confirm_password" placeholder="Enter Confirmed Password" minlength="8" maxlength="16" required>
                    <label class="show-confirm-password-container">
                    <input type="checkbox" checked="checked" v-model="showConfirmPassword" required>
                      <svg class="eye" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512"><path d="M288 32c-80.8 0-145.5 36.8-192.6 80.6C48.6 156 17.3 208 2.5 243.7c-3.3 7.9-3.3 16.7 0 24.6C17.3 304 48.6 356 95.4 399.4C142.5 443.2 207.2 480 288 480s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C433.5 68.8 368.8 32 288 32zM144 256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64 64c-7.1 0-13.9-1.2-20.3-3.3c-5.5-1.8-11.9 1.6-11.7 7.4c.3 6.9 1.3 13.8 3.2 20.7c13.7 51.2 66.4 81.6 117.6 67.9s81.6-66.4 67.9-117.6c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3z"></path></svg>
                      <svg class="eye-slash" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 640 512"><path d="M38.8 5.1C28.4-3.1 13.3-1.2 5.1 9.2S-1.2 34.7 9.2 42.9l592 464c10.4 8.2 25.5 6.3 33.7-4.1s6.3-25.5-4.1-33.7L525.6 386.7c39.6-40.6 66.4-86.1 79.9-118.4c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C465.5 68.8 400.8 32 320 32c-68.2 0-125 26.3-169.3 60.8L38.8 5.1zM223.1 149.5C248.6 126.2 282.7 112 320 112c79.5 0 144 64.5 144 144c0 24.9-6.3 48.3-17.4 68.7L408 294.5c8.4-19.3 10.6-41.4 4.8-63.3c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3c0 10.2-2.4 19.8-6.6 28.3l-90.3-70.8zM373 389.9c-16.4 6.5-34.3 10.1-53 10.1c-79.5 0-144-64.5-144-144c0-6.9 .5-13.6 1.4-20.2L83.1 161.5C60.3 191.2 44 220.8 34.5 243.7c-3.3 7.9-3.3 16.7 0 24.6c14.9 35.7 46.2 87.7 93 131.1C174.5 443.2 239.2 480 320 480c47.8 0 89.9-12.9 126.2-32.5L373 389.9z"></path></svg>
                    </label>
                </div>
                <button type="submit" class="btn login-btn mb-2" @click="submitChangePassword">Submit</button>
              
              </form>
             
        </div>
        </div>
        <div class="row align-items-center justify-content-center flex-column" v-else>
            <div class="form-input" v-if="!verification">
              <h2>Send Verification</h2>
                <form @submit.prevent="sendOtp">
                <div class="form-group">
                    <label for="username">Contact Number:</label>
                    <input type="text" class="form-control" v-model="contactNumber" readonly> 
                </div>
                <button type="submit" class="btn login-btn mb-2">Send OTP</button>
              </form>
            </div>
            
            <div v-else>
                <form class="form otp-form" @submit.prevent="sendVerification" > 
                <div class="title">OTP</div> 
                <div class="title">Verification Code</div> 
                <p class="message">We have sent a verification code to your mobile number</p> 
                <div class="inputs"> 
                  <input id="input1" type="text" v-model="oneTimePassword[0]" maxlength="1"> 
                  <input id="input2" type="text" v-model="oneTimePassword[1]" maxlength="1"> 
                  <input id="input3" type="text" v-model="oneTimePassword[2]" maxlength="1"> 
                  <input id="input4" type="text" v-model="oneTimePassword[3]" maxlength="1"> 
                  <input id="input5" type="text" v-model="oneTimePassword[4]" maxlength="1"> 
                  <input id="input6" type="text" v-model="oneTimePassword[5]" maxlength="1"> 
                </div> 
                <div>
                  <div class="count-down-timer">
                    <span>{{ Math.floor(countdownTime / 60) }}:</span>
                    <span>{{ countdownTime % 60 < 10 ? '0' : '' }}{{ countdownTime % 60 }}</span>
                  </div>
                  <div> <button class="action" type="submit">Submit OTP</button></div>
                </div>
                
              </form>
            </div>
        </div>
      


      </div>
    </div>
</template>

<style>
/*------ Settings ------*/
.password{
  position: relative;
}
.showpassword-container {
  --color: #a5a5b0;
  --size: 30px;
  display: flex;
  justify-content: start;
  align-items: center;
  position: absolute;
  cursor: pointer;
  font-size: var(--size);
  user-select: none;
  fill: var(--color);
  top: 50px;
  right: 30px;
}

.showpassword-container .eye {
  position: absolute;
  animation: keyframes-fill .5s;
}

.showpassword-container .eye-slash {
  position: absolute;
  animation: keyframes-fill .5s;
  display: none;
}

/* ------ On check event ------ */
.showpassword-container input:checked ~ .eye {
  display: none;
}

.showpassword-container input:checked ~ .eye-slash {
  display: block;
}

.showpassword-container input:checked ~ .eye-confirm {
  display: none;
}

.showpassword-container input:checked ~ .eye-slash-confirm {
  display: block;
}

/* ------ Hide the default checkbox ------ */
.showpassword-container input {
  position: absolute;
  opacity: 0;
  cursor: pointer;
  height: 0;
  width: 0;
}


/* confirm  */
.show-confirm-password-container {
  --color: #a5a5b0;
  --size: 30px;
  display: flex;
  justify-content: start;
  align-items: center;
  position: absolute;
  cursor: pointer;
  font-size: var(--size);
  user-select: none;
  fill: var(--color);
  top: 50px;
  right: 30px;
}

.show-confirm-password-container .eye {
  position: absolute;
  animation: keyframes-fill .5s;
}

.show-confirm-password-container .eye-slash {
  position: absolute;
  animation: keyframes-fill .5s;
  display: none;
}

/* ------ On check event ------ */
.show-confirm-password-container input:checked ~ .eye {
  display: none;
}

.show-confirm-password-container input:checked ~ .eye-slash {
  display: block;
}

.show-confirm-password-container input:checked ~ .eye-confirm {
  display: none;
}

.show-confirm-password-container input:checked ~ .eye-slash-confirm {
  display: block;
}

/* ------ Hide the default checkbox ------ */
.show-confirm-password-container input {
  position: absolute;
  opacity: 0;
  cursor: pointer;
  height: 0;
  width: 0;
}

.try-another{
  cursor: pointer;
  font-weight: 600;
  font-size: 12px;
}

/* confirm */

/* ------ Animation ------ */
@keyframes keyframes-fill {
  0% {
    transform: scale(0);
    opacity: 0;
  }

  50% {
    transform: scale(1.2);
  }
}
</style>

